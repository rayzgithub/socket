...
class redisClient{
   ...
   * @return boolean
   */
  public function connect($config=array('host'=>'192.168.1.31','port'=>6379), $isMaster=true){
    // default port
  ...
  public function get($key){
    ...
    if(!$this->_isUseCluster){
	    ...
	    // 使用了 M/S
	    $result = $this->_getSlaveRedis()->{$func}($key);
    }
    if(is_array($result)){
    	foreach($result as $key=>$val){
    		$result[$key] = $this->cache_decode($val);
    	}
    }else{
    	$result = $this->cache_decode($result);
    }
    return $result;
  }
  ...
  */
   ...
   * @return string
   * **/
  private function cache_encode($val){
...
/**
   ...
   * @return string
   * **/
  private function cache_decode($val){
  ...
  public function lpop($key){
    $result = $this->getRedis()->lpop($key);
    return $this->cache_decode($result);
  }
  /**
   * lrange
   */
